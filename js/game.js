...
Game.prototype.draw = function() {
  ...
  this.clouds.forEach(function(nimbus) {
    nimbus.draw();
  });
  this.plane.draw();
  this.minions.forEach(
    function(minion) {
      minion.draw(this.waveSize);
    }.bind(this)
  );
  this.scoring.draw();
  this.packs.forEach(function(pack) {
...
Game.prototype.eliminateBalls = function() {
  for (i = 0; i < this.plane.cannonballs.length; i++) {
    if (this.plane.cannonballs[i].y < 0) {
      this.plane.cannonballs.splice(
        this.plane.cannonballs.indexOf(this.plane.cannonballs[i], 1)
      );
    }
  }
};
...
Game.prototype.gameOverScreen = function() {
  ...
  ctx.fillText(
    "The gnome airship was destroyed!",
    width / 4,
    height / 3.5,
    width / 2,
    300
...
Game.prototype.startScreen = function() {
  ...
  splash.onload = function() {
    ...
    ctx.shadowColor = "black";
    ctx.fillRect(width / 8, height / 8, 900, 500);
    ctx.drawImage(splash, width / 8, height / 8, 900, 500);
    ctx.restore();
    ctx.strokeStyle = "#ffffb3";
    ctx.lineWidth = 10;
    ctx.strokeRect(width / 8, height / 8, 900, 500);
    ...
    document.fonts.load('10pt "Carousel"').then(renderText);
    function renderText() {
      ctx.save();
      ctx.font = "small-caps bold 70pt Carousel";
      ctx.fillStyle = "goldenrod";
      ctx.setTransform (1.25, -0.1, -0.2, 1.2, -125, 50);
      ctx.fillText(
        "Gnome adventure of Dragons!",
        width / 4,
        height / 3.5,
        width / 2,
        300
